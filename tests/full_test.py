...
class FullTestLabeledSketch:
    ...
    def test_all_distance_sketch(self, network, k, rnd_labels) -> snap.TFltV:
        ...
        self.test_tracker.start()
        bottom_k_estimates = snap.TFltV()
        for (node_id, labels)  in rnd_labels:
            estimate = graph_sketch.cardinality_estimation_labels_node_id(node_id, labels)
    ...
    def calculate_tc_labels(self, network, rnd_labels):
        ...
        self.test_tracker.start()
        for (node_id, labels) in rnd_labels:
            tc = self.transitive_closure(network, node_id, labels)
            tc_values.append(tc)
        tc_time = self.test_tracker.track()
    ...
    def full_test_network(self, network) -> pd.DataFrame:
        ...
        for i in range(self.N):
            ...
            node_id = network.GetRndNId(rnd)
            node_id_label_samples.append((node_id, label_sample))

        node_results['Labels'] = node_id_label_samples

        if self.calc_tc:
            tc_values = self.calculate_tc_labels(network, node_id_label_samples)
            node_results['TC'] = tc_values

        if self.test_sketch:
            for k in self.k_values:
                sketch_estimates = self.test_all_distance_sketch(
                    network, k, node_id_label_samples)
    ...
    def start_full_test(self, max_files_tested=inf, num_files_skip=0):
        # Get the data files and their sizes in the data directory
        files_roots_sizes = {}
        for root, dirs, files in os.walk("data/labeled/real"):
            for file in files:
        ...
        for file, (root, size) in sorted(files_roots_sizes.items(), key=lambda item: item[1]):
            ...
            node_results_df = self.full_test_network(network)
            if self.calc_tc or self.test_sketch:
                node_results_filename = f"results/{self.test_timestamp}_{file}_node_results_N={self.N}.csv"
                node_results_df.to_csv(node_results_filename, index=False)
...
class FullTestLabeledSummary:
    ...
    def __init__(self, calc_tc=True, test_summary=True, seed=42, N=1000):
        self.calc_tc = calc_tc
        self.test_summary = test_summary
        self.seed = seed
        self.N = N
    ...
    def test_graph_merge_summary(self, network, labels) -> dict:
        merge_types = [False, True]
        ...
        for merge_type in merge_types:
            test_print(f"Creating merge summary is_target_merge={merge_type}...")
            self.test_tracker.start()
            summary.build_merge_network(is_target_merge=merge_type)
            merge_time = (self.test_tracker.track())
            summary_n_nodes = summary.merge_network.GetNodes()
            summary_n_edges = summary.merge_network.GetEdges()
            test_print(f"Finished creating merge summary is_target_merge={merge_type}.")

            # Perform estimates on summary
            test_print(f"Estimating for N={self.N} on merge summary is_target_merge={merge_type}...")
            self.test_tracker.start()
            estimates = []
            for label in labels:
                estimate = summary.cardinality_estimation_label(label)
                estimates.append(estimate)
            merge_estimates[merge_type] = estimates
            est_time = self.test_tracker.track()
            test_print(f"Finished estimating for N={self.N} on merge summary is_target_merge={merge_type}.")
    ...
    def transitive_closure(self, graph, root_node_id, label):
        queue = snap.TIntV()
        queue.append(root_node_id)
        visited_nodes = snap.TIntSet()
        visited_nodes.AddKey(root_node_id)
    ...
    def full_transitive_closure(self, graph, label):
        ...
        for NI in graph.Nodes():
            node_id = NI.GetId()
            tc += self.transitive_closure(graph, node_id, label)
        return tc

    def calculate_tc_labels(self, network, labels):
        test_print(f"Calculating TC for N={self.N}...")
        tc_values = snap.TIntV()
        self.test_tracker.start()
        for label in labels:
            test_print(f'Label: {label}...')
            tc = self.full_transitive_closure(network, label)
            tc_values.append(tc)
        tc_time = self.test_tracker.track()
    ...
    def full_test_network(self, network) -> pd.DataFrame:
        ...
        if self.calc_tc:
            tc_values = self.calculate_tc_labels(network, labels)
            node_results['TC'] = tc_values
    ...
    def start_full_test(self, max_files_tested=inf, num_files_skip=0):
        ...
        for file, (root, size) in sorted(files_roots_sizes.items(), key=lambda item: item[1]):
            ...
            node_results_df = self.full_test_network(network)
            if self.calc_tc or self.test_summary:
                node_results_filename = f"results/{self.test_timestamp}_{file}_node_results_N={self.N}.csv"
                node_results_df.to_csv(node_results_filename, index=False)
